...
- **YouTube タブ**: URL入力時にサムネイルプレビューを即時表示（ネットワーク不通時はフォールバックUI）
- **YouTube タブ**: 「ベスト画質」モード追加（720p以上の映像+音声を自動マージ）
- **フォント**: Google Fonts → システムフォント（Segoe UI / Meiryo）に変更し、オフライン環境でも高速起動
- **ドラッグ&ドロップ**: 全タブで Tauri `onDragDropEvent` API 対応（フルパス取得）
- **DropZone**: クリック時に Tauri ファイルダイアログを使用するように改善
- **カット・分離タブ**: ファイル選択後にUI（タイムライン・ストリーム一覧）が表示されない問題を修正
